                ...
                <div class="col-md-12">
                    ...
                    <div class="card card-info">
                        <div class="card-header" style="background-color: rgb(102, 170, 102)">
                            <h3 class="card-title">Edit Plant Identification</h3>
                        </div>
                        <!-- /.card-header -->
                        <!-- form start -->
                        <form action="{{ route('plant_identifications.update', $plantIdentification->id) }}" method="POST"
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control input-border select2" name="plant_category_id">
                                                ...
                                                @foreach ($plantCategories as $plantCategory)
                                                    <option value="{{ $plantCategory->id }}"
                                                        {{ old('plant_category_id', $plantIdentification->plant_category_id == $plantCategory->id ? 'selected' : '') }}>
...
@section('script')
    ...
    <script>
        @if ($plantIdentification->image)
            ...
            myDropzone.emit("thumbnail", mockFile, "{{ asset('storage/' . $plantIdentification->image) }}");
            myDropzone.emit("complete", mockFile);
            document.getElementById('uploaded_image').value = "{{ $plantIdentification->image }}";
        @endif
    </script>
@endsection
